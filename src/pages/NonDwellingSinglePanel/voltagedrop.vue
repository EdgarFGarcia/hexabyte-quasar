    ...
    <q-page-container v-if="getisvoltagedrop">
        <q-card-section class="text-center">
            ...
            <div class="row" style="margin-top: 10px;">
                ...
                <div class="col-8">
                    <label v-if="Object.keys(getnav_voltage_drop).length !== 0">{{getnav_voltage_drop.voltagedrop}}</label>
                </div>
                <div class="col-4" style="background: black !important;">
                    <label style="color: white;">VD% (%)</label>
                </div>
                <div class="col-8">
                    <label v-if="Object.keys(getnav_voltage_drop).length !== 0">{{getnav_voltage_drop.voltage_drop_percent}}</label>
                </div>
        ...
        <q-card-section class="text-center">
            ...
            <div class="row" style="margin-top: 10px;">
                ...
                <div class="col-8">
                    <label v-if="Object.keys(getsupply_to_branch).length !== 0">{{getsupply_to_branch.voltagedrop}}</label>
                </div>
                <div class="col-4" style="background: black !important;">
                    <label style="color: white;">VD% (%)</label>
                </div>
                <div class="col-8">
                    <label v-if="Object.keys(getsupply_to_branch).length !== 0">{{getsupply_to_branch.voltage_drop_percent}}</label>
                </div>
                <div class="col-12" style="margin-top: 15px;">
...
export default {
  ...
  mounted(){
    console.log('called mounted short circuit non dwelling')
    this.$store.dispatch('topnav/setshowddcalc', false)
    this.$store.dispatch('topnav/setnavigationchild', 'VOLTAGE DROP')
    this.$store.dispatch('topnav/setisvoltagedrop', true)
  },
  computed : {
    ...mapGetters({
        ...
        getnav_voltage_drop:    'ndsingle/getnav_voltage_drop',
        getsupply_to_branch:    'ndsingle/getsupply_to_branch'
    })
  },
  methods : {
    calculatemainfeeder(){
        this.$store.dispatch('ndsingle/voltagedropmainfeeder', this.voltagedrop.length)
    },
    calculatesupplytobranch(){
        // console.log(this.supplytobranch)
        this.$store.dispatch('ndsingle/supplytobranch', this.supplytobranch)
    }
  }
}
</script>
